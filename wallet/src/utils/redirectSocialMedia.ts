...
import {LTO_WALLET_URL} from '@env';
...
export const navigateToExplorer = async () => {
  const explorerUrl = WalletNetworkService.getExplorerBaseUrl();
  if (explorerUrl) {
...
export const navigateToTransaction = async (id: string) => {
  const transactionUrl = sanitizeUrl(WalletNetworkService.getExplorerTxUrl(id));
  if (transactionUrl) {
